 ...
 *
 * Kept separate from `client.ts` so the cross-cutting concerns —
 * auth-header injection, query serialization, dev logging — are composable
...
export function logRequest(method: string, url: string, tag: object): void {
  ...
  console.debug(`%c→ ${method} ${url}`, "color:#888");
}
...
export function logResponse(method: string, url: string, status: number, tag: object): void {
  ...
  console.debug(
    `%c← ${status} ${method} ${url}${ms != null ? ` (${ms}ms)` : ""}`,
    `color:${color}`,
  );
}
