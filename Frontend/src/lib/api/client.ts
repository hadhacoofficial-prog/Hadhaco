 ...
 *   • Base URL join + query serialization
 *   • Authorization: Bearer <Supabase JWT> injection (read fresh per request)
 *   • Response-envelope unwrapping → callers get `T`, never `{ data }`
 *   • Error normalization → always throws `ApiError`
 *   • Multipart (FormData) support
 *   • Timeout (AbortController) + retry with backoff
  ...
  while (true) {
    ...
    try {
      ...
      if (res.status === 401 && !options.skipAuth && !_retried) {
        ...
        if (data.session) {
          // Fresh session obtained — retry once with the new token.
...
export const api = {
  ...
  /** Multipart helper — pass a FormData body; Content-Type is set by the browser. */
