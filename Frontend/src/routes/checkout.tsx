...
import { Truck, Tag, Plus } from "lucide-react";
...
export const Route = createFileRoute("/checkout")({
  beforeLoad: async () => {
    if (typeof window === "undefined") return; // SSR: localStorage unavailable, skip server-side redirect
...
function CheckoutPage() {
  ...
  const createPaymentMutation = useMutation({
    mutationFn: async (body: CreatePaymentIntentRequest) => {
      // Sync local cart to server cart first (preserving variant selections)
  ...
  const verifyPaymentMutation = useMutation({
    ...
    onSuccess: (result) => {
      // Clear cart and bust all stale caches before navigating away
      clear();
    ...
    onError: (e) => {
      toast.error(`Payment verification failed: ${toUserMessage(e)}. Please contact support.`);
    },
  });
        ...
        <form onSubmit={placeOrder} className="grid lg:grid-cols-[1fr_400px] gap-10">
            ...
            {selectedAddressId === "new" && (
              <Section title={addresses.length > 0 ? "New address" : "Shipping address"}>
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <Field label="Apartment, suite (optional)" name="apt" className="md:col-span-2" />
          ...
          <aside className="border border-border bg-card p-6 h-fit lg:sticky lg:top-28">
            ...
            <button
              ...
              className="mt-5 w-full bg-primary text-primary-foreground text-[11px] uppercase tracking-[0.22em] py-3.5 hover:bg-accent hover:text-accent-foreground transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {verifyPaymentMutation.isPending
                ? "Confirming payment…"
                : createPaymentMutation.isPending || createAddressMutation.isPending
                  ? "Preparing…"
                  : "Place Order"}
            </button>
