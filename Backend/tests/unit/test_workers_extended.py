...
class TestShipmentSyncWorker:
    ...
    async def test_run_handles_sync_exception_gracefully(self):
        ...
        with patch("app.workers.shipment_sync.AsyncSessionLocal", return_value=mock_db), \
             patch("app.workers.shipment_sync._shipping.sync_shipment_status", AsyncMock(side_effect=RuntimeError("timeout"))):
...
class TestInventoryAlertsWorker:
    async def test_run_with_no_low_stock_products(self):
        ...
        with patch("app.workers.inventory_alerts.AsyncSessionLocal", return_value=mock_db), \
...
class TestNotificationRetryWorker:
    async def test_run_calls_retry_pending(self):
        ...
        with patch("app.workers.notification_retry.AsyncSessionLocal", return_value=mock_db), \
...
class TestReviewReminderWorker:
    async def test_run_with_no_candidates(self):
        ...
        with patch("app.workers.review_reminder.AsyncSessionLocal", return_value=mock_db), \
...
class TestAbandonedCartWorker:
    async def test_run_returns_early_when_feature_disabled(self):
        ...
        with patch("app.workers.abandoned_cart.AsyncSessionLocal", return_value=mock_db), \
             patch("app.modules.settings.service.SettingsService.is_feature_enabled", AsyncMock(return_value=False)):
            await worker.run()
    ...
    async def test_run_with_no_carts_when_feature_enabled(self):
        ...
        with patch("app.workers.abandoned_cart.AsyncSessionLocal", return_value=mock_db), \
             patch("app.modules.settings.service.SettingsService.is_feature_enabled", AsyncMock(return_value=True)):
            await worker.run()
