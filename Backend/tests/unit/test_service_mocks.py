...
class TestCMSService:
    ...
    async def test_get_home_data_returns_required_keys(self):
        ...
        with (
            patch(
                "app.modules.cms.repository.CMSRepository.get_active_banners",
                AsyncMock(return_value=[]),
            ),
            patch(
                "app.modules.cms.repository.CMSRepository.get_active_sections",
                AsyncMock(return_value=[]),
            ),
        ):
    ...
    async def test_update_section_raises_404_when_key_not_found(self):
        ...
        db = AsyncMock()
        with patch(
            "app.modules.cms.repository.CMSRepository.get_section_by_key",
            AsyncMock(return_value=None),
        ):
            with pytest.raises(HTTPException) as exc:
...
class TestCategoryService:
    ...
    async def test_delete_raises_404_when_not_found(self):
        ...
        db = AsyncMock()
        with patch(
            "app.modules.categories.repository.CategoryRepository.get_by_id",
            AsyncMock(return_value=None),
        ):
    ...
    async def test_get_by_slug_raises_404_when_missing(self):
        ...
        db = AsyncMock()
        with patch(
            "app.modules.categories.repository.CategoryRepository.get_by_slug",
            AsyncMock(return_value=None),
        ):
            with pytest.raises(NotFoundError):
