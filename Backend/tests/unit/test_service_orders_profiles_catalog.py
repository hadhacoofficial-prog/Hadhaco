...
class TestOrderServiceGetAndList:
    ...
    async def test_get_order_returns_validated_response(self):
        ...
        mock_order = MagicMock()
        with patch("app.modules.orders.service._repo.get_by_id", AsyncMock(return_value=mock_order)), \
             patch("app.modules.orders.service.OrderResponse.model_validate", return_value=MagicMock()):
...
class TestOrderServiceUpdateStatus:
    ...
    async def test_update_status_publishes_event_and_returns_response(self):
        ...
        with patch("app.modules.orders.service._repo.get_by_id", AsyncMock(return_value=mock_order)), \
             ...
             patch("app.modules.orders.service.OrderResponse.model_validate", return_value=MagicMock()):
            result = await self.svc.update_status(db, uuid.uuid4(), UpdateOrderStatusRequest(status="confirmed"))
        assert result is not None
    ...
    async def test_update_status_to_delivered_sets_delivered_at(self):
        ...
        with patch("app.modules.orders.service._repo.get_by_id", AsyncMock(return_value=mock_order)), \
             ...
             patch.object(event_bus, "publish", AsyncMock()), \
             patch("app.modules.orders.service.OrderResponse.model_validate", return_value=MagicMock()):
            await self.svc.update_status(db, uuid.uuid4(), UpdateOrderStatusRequest(status="delivered"))
...
class TestOrderServiceCancel:
    ...
    async def test_cancel_raises_validation_error_when_already_delivered(self):
        ...
        with patch("app.modules.orders.service._repo.get_by_id", AsyncMock(return_value=mock_order)):
            with pytest.raises(ValidationError):
                await self.svc.cancel_order(db, uuid.uuid4(), user_id, CancelOrderRequest(reason="Changed mind"))
    ...
    async def test_cancel_success_returns_response(self):
        ...
        with patch("app.modules.orders.service._repo.get_by_id", AsyncMock(return_value=mock_order)), \
             ...
             patch.object(event_bus, "publish", AsyncMock()), \
             patch("app.modules.orders.service.OrderResponse.model_validate", return_value=MagicMock()):
            result = await self.svc.cancel_order(db, uuid.uuid4(), user_id, CancelOrderRequest(reason="Changed mind"))
        assert result is not None
...
class TestProfileService:
    ...
    async def test_update_profile_raises_404_when_not_found(self):
        ...
        with patch.object(self.repo_cls, "update", AsyncMock(return_value=None)):
            with pytest.raises(NotFoundError):
                await self.svc.update_profile(db, str(uuid.uuid4()), ProfileUpdateRequest(full_name="Alice"))
    ...
    async def test_update_avatar_raises_404_when_not_found(self):
        ...
        with patch.object(self.repo_cls, "update", AsyncMock(return_value=None)):
            with pytest.raises(NotFoundError):
                await self.svc.update_avatar(db, str(uuid.uuid4()), "https://cdn.example.com/avatar.jpg")
    ...
    async def test_update_avatar_returns_updated_profile(self):
        ...
        with patch.object(self.repo_cls, "update", AsyncMock(return_value=mock_profile)):
            result = await self.svc.update_avatar(db, str(uuid.uuid4()), "https://cdn.example.com/avatar.jpg")
        assert result is mock_profile

    async def test_list_users_returns_paginated(self):
        from datetime import datetime, timezone
        from app.modules.profiles.schemas import AdminUserListItem
        db = AsyncMock()
        mock_profile = MagicMock()
        ...
        with patch.object(self.repo_cls, "list_paginated", AsyncMock(return_value=([mock_profile], 1))):
    ...
    async def test_change_role_success_calls_audit(self):
        ...
        with patch.object(self.repo_cls, "get_by_id", AsyncMock(return_value=mock_profile)), \
            ...
            result = await self.svc.change_role(db, str(uuid.uuid4()), UserRole.ADMIN, str(uuid.uuid4()))
...
class TestCatalogServiceRead:
    ...
    async def test_get_by_id_returns_validated_response(self):
        ...
        with patch("app.modules.catalog.service._repo.get_by_id", AsyncMock(return_value=mock_product)), \
             patch("app.modules.catalog.service.ProductResponse.model_validate", return_value=MagicMock()):
...
class TestCatalogServiceVariantsAndAttributes:
    ...
    async def test_add_variant_raises_404_when_product_not_found(self):
        ...
        with patch("app.modules.catalog.service._repo.get_by_id", AsyncMock(return_value=None)):
            with pytest.raises(NotFoundError):
                await self.svc.add_variant(db, uuid.uuid4(), ProductVariantCreateRequest(sku="V-001", name="Small"))

    async def test_add_variant_raises_conflict_on_duplicate_sku(self):
        from app.core.exceptions import ConflictError
        from app.modules.catalog.schemas import ProductVariantCreateRequest
        db = AsyncMock()
        with patch("app.modules.catalog.service._repo.get_by_id", AsyncMock(return_value=MagicMock())), \
             patch("app.modules.catalog.service._repo.get_by_sku", AsyncMock(return_value=MagicMock())):
            with pytest.raises(ConflictError):
                await self.svc.add_variant(db, uuid.uuid4(), ProductVariantCreateRequest(sku="V-001", name="Small"))
    ...
    async def test_add_variant_success(self):
        ...
        with patch("app.modules.catalog.service._repo.get_by_id", AsyncMock(return_value=MagicMock())), \
             ...
             patch("app.modules.catalog.service._repo.add_variant", AsyncMock(return_value=mock_variant)):
            result = await self.svc.add_variant(db, uuid.uuid4(), ProductVariantCreateRequest(sku="V-001", name="Small"))
    ...
    async def test_update_variant_raises_404(self):
        ...
        with patch("app.modules.catalog.service._repo.get_variant", AsyncMock(return_value=None)):
            with pytest.raises(NotFoundError):
                await self.svc.update_variant(db, uuid.uuid4(), ProductVariantUpdateRequest(name="Updated"))
    ...
    async def test_upsert_attribute_raises_404_when_product_not_found(self):
        ...
        with patch("app.modules.catalog.service._repo.get_by_id", AsyncMock(return_value=None)):
            with pytest.raises(NotFoundError):
                await self.svc.upsert_attribute(db, uuid.uuid4(), ProductAttributeCreateRequest(name="metal", value="Silver"))
