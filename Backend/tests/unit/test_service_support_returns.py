...
class TestSupportServiceReply:
    ...
    async def test_reply_raises_404_when_ticket_missing(self):
        ...
        with patch.object(SupportRepository, "get_ticket", AsyncMock(return_value=None)):
            with pytest.raises(HTTPException) as exc:
                await self.svc.reply(db, ticket_id=uuid.uuid4(), sender_id=uuid.uuid4(), data=MessageCreate(body="hi"))
    ...
    async def test_reply_raises_403_when_not_customers_ticket(self):
        ...
        with patch.object(SupportRepository, "get_ticket", AsyncMock(return_value=mock_ticket)):
            with pytest.raises(HTTPException) as exc:
                await self.svc.reply(db, ticket_id=uuid.uuid4(), sender_id=caller_id, data=MessageCreate(body="hi"), is_admin=False)
    ...
    async def test_reply_customer_reopens_resolved_ticket(self):
        ...
        with patch.object(SupportRepository, "get_ticket", AsyncMock(return_value=mock_ticket)), \
            ...
            result = await self.svc.reply(db, ticket_id=ticket_id, sender_id=customer_id, data=MessageCreate(body="Still broken"))
    ...
    async def test_admin_reply_bypasses_customer_check(self):
        ...
        with patch.object(SupportRepository, "get_ticket", AsyncMock(return_value=mock_ticket)), \
            ...
            db.commit = AsyncMock()
            db.refresh = AsyncMock()
            result = await self.svc.reply(db, ticket_id=ticket_id, sender_id=admin_id, data=MessageCreate(body="Noted"), is_admin=True)
...
class TestSupportServiceGetTicket:
    ...
    async def test_admin_update_raises_404_when_not_found(self):
        ...
        with patch.object(SupportRepository, "get_ticket", AsyncMock(return_value=None)):
            with pytest.raises(HTTPException) as exc:
                await self.svc.admin_update(db, ticket_id=uuid.uuid4(), data=AdminTicketUpdate(status="closed"))
        assert exc.value.status_code == 404
...
class TestReturnService:
    ...
    async def test_create_return_raises_400_when_outside_window(self):
        db = AsyncMock()
        with patch.object(ReturnRepository, "is_within_return_window", AsyncMock(return_value=False)):
            with pytest.raises(HTTPException) as exc:
                await self.svc.create_return(
                    ...
                    data=ReturnCreate(
                        order_id=uuid.uuid4(),
                        reason="defective",
                        items=[ReturnItemCreate(order_item_id=uuid.uuid4(), quantity=1, reason="defective")],
                    ),
                )
    ...
    async def test_create_return_success(self):
        ...
        with patch.object(ReturnRepository, "is_within_return_window", AsyncMock(return_value=True)), \
    ...
    async def test_admin_update_status_success(self):
        ...
        with patch.object(ReturnRepository, "get", AsyncMock(return_value=mock_ret)), \
