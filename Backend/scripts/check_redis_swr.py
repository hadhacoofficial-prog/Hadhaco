...
for key in sorted(r.keys("*")):
    ...
    print(
        f"  {key:40s} TTL={ttl:>6d}s  size={orig_size:>7,d}B  compressed={is_compressed}  swr={has_swr}  age={age:>6.0f}s  {extra}"
    )
...
for key in sorted(r.keys("*")):
    ...
    if is_compressed:
        try:
            ...
            print(
                f"  {key:40s} compressed={len(val):>6,d}B  decompressed={decomp_size:>7,d}B  ratio={ratio:.1f}x"
            )
