...
def flush_redis():
    ...
    subprocess.run(
        ["docker", "exec", "hadha-redis", "redis-cli", "FLUSHDB"], capture_output=True
    )
...
items = r1["data"]["items"]
print(
    f"Cold product list: {elapsed_ms:.1f}ms, {len(items)} items, total={r1['data']['total']}"
)
...
print("  1. Product data + count (window function)")
print("  2. Image IDs via CTE (primary+secondary per product)")
print("  3. Full Image objects (batch by IDs)")
print("  4. Image variants (batch for fetched images)")
print("  5. Product variants (selectinload for available_stock)")
print("  6. Collections batch")
print(
    "  = 6 queries total (BEFORE was 6+ = count + images + image_variants + variants + collections)"
)
print("  BUT: count is merged into query 1, images are limited to 2/product")
