...
async def create_ticket(
    ...
    return created(
        result,
        ResponseCode.SUPPORT_TICKET_CREATED,
...
@router.patch(
    "/admin/tickets/{ticket_id}", response_model=BaseSuccessResponse[TicketOut]
)
async def admin_update_ticket(
    ...
    return ok(
        result, ResponseCode.SUPPORT_TICKET_UPDATED, "Ticket updated successfully"
    )


@router.post(
    "/admin/tickets/{ticket_id}/messages", response_model=BaseSuccessResponse[dict]
)
async def admin_reply_ticket(
    ...
    result = await _svc.reply(
        db, ticket_id=ticket_id, sender_id=admin.id, data=data, is_admin=True
    )
    return ok(result, ResponseCode.SUPPORT_TICKET_REPLIED, "Reply sent successfully")
