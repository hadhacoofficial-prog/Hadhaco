...
class SupportService:
    ...
    async def create_ticket(self, db: AsyncSession, *, customer_id: uuid.UUID, data: TicketCreate) -> SupportTicket:
    ...
    async def reply(self, db: AsyncSession, *, ticket_id: uuid.UUID, sender_id: uuid.UUID, data: MessageCreate, is_admin: bool = False) -> SupportMessage:
        ...
        msg = await self._repo.add_message(
            db, ticket_id=ticket_id, sender_id=sender_id, body=data.body, is_internal=data.is_internal and is_admin
        )
    ...
    async def list_customer_tickets(self, db: AsyncSession, customer_id: uuid.UUID) -> list[SupportTicket]:
        return await self._repo.list_for_customer(db, customer_id)

    async def get_ticket(self, db: AsyncSession, ticket_id: uuid.UUID, *, viewer_id: uuid.UUID, is_admin: bool) -> SupportTicket:
        ticket = await self._repo.get_ticket(db, ticket_id)
        if not ticket:
            raise HTTPException(status.HTTP_404_NOT_FOUND, "Ticket not found")
    ...
    async def admin_list(self, db: AsyncSession, *, status_filter: str | None, offset: int, limit: int) -> list[SupportTicket]:
        return await self._repo.list_all(db, status=status_filter, offset=offset, limit=limit)

    async def admin_update(self, db: AsyncSession, *, ticket_id: uuid.UUID, data: AdminTicketUpdate) -> SupportTicket:
        ticket = await self._repo.get_ticket(db, ticket_id)
        if not ticket:
            raise HTTPException(status.HTTP_404_NOT_FOUND, "Ticket not found")
